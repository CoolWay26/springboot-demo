...
public class PoiUtils2 {
    private static Logger logger = LoggerFactory.getLogger(PoiUtils2.class);
    ...
    public static List<Map<String, Object>> readExcel(InputStream file, String fileName, int starRow, int starCell, String[] fieldEnglish) {
        Workbook wb = null;
        //判断文件类型 03或是07
        try {
            if (isExcel2007(fileName)) {
        ...
        try {
            ...
            for (int i = starRow; i <= lastRowNum; i++) {
                ...
                for (int j = starCell, fieldIndex = 0; j < lastCellNum; j++, fieldIndex++) {
                    Cell cell = row.getCell(j);
                    //获取当前cell表格的值,并放入obj
                    mapTemp.put(fieldEnglish[fieldIndex], getMyCellType(cell));
                }
                list.add(mapTemp);
            }
            wb.close();
